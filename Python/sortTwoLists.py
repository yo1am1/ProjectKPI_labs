"""
Програмування частина 2. Лабораторна робота №1. name group. Варіант 10 (23 № у списку)
"""
...
def random():
    ...
    list_0, list_1 = [ran(-100, 100) for _ in range(ran(5, 20))], [
        ran(-100, 100) for _ in range(ran(5, 20))
    ]
    ...
    try:
        while True:
            ...
            list_0.append(
                int(
                    input(
                        'Input an element of the list "A" (input any other key to stop inserting elements): '
                    )
                )
            )
    except ValueError:
    ...
    try:
        while True:
            ...
            list_1.append(
                int(
                    input(
                        'Input an element of the list "B" (input any other key to stop inserting elements): '
                    )
                )
            )
    except ValueError:
...
def loop():
    ...
    while True:
        ...
        lista.extend(list_1)
        print(
            "\nSorted result (merged lists):",
            list(dict.fromkeys(merge_lists(sorted(list_0), sorted(list_1)))),
        )
